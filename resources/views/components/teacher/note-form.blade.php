...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data" data-teacher-note-form>
    @csrf
    ...
    <div data-teacher-image-payload-inputs></div>
    ...
    <div class="animate-slide-up rounded-[2rem] border border-amber-200 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] ring-2 ring-amber-100">
        <x-shared.rich-editor
            ...
            id="teacher-note-editor"
        />

        <div class="hidden px-6 pb-4" data-teacher-image-preview-list></div>
        ...
        <div class="border-t border-slate-100 px-4 py-4">
            <div class="flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-xs font-semibold tracking-[0.18em] text-slate-400" data-teacher-note-count>0/5000</span>
                </div>

                <div class="flex items-center gap-2">
                    @if ($isEditing)
                        <a href="{{ route('teacher.students.show', data_get($note, 'student_id', $studentId)) }}" class="inline-flex h-10 items-center rounded-full px-4 text-sm font-semibold text-slate-500 transition-colors hover:bg-slate-100">Batal</a>
                    @else
                        <button type="button" class="inline-flex h-10 items-center rounded-full px-4 text-sm font-semibold text-slate-500 transition-colors hover:bg-slate-100" data-teacher-composer-close>Batal</button>
                    @endif
