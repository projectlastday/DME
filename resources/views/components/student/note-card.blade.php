...
@php
    ...
    $authorName = data_get($note, 'author_name_snapshot', 'Bapak/Ibu Guru');
    $authorRole = (string) data_get($note, 'author_role_snapshot', 'author');
    $isEditableStudentNote = (string) data_get($note, 'author_id') === (string) auth()->id()
        && \App\Models\User::roleMatches($authorRole, \App\Models\User::ROLE_STUDENT);
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        ...
        @if ($isEditableStudentNote)
            <div class="flex shrink-0 items-center gap-5 text-slate-400">
                <a href="{{ route('student.notes.edit', data_get($note, 'id')) }}" class="rounded-full p-1 text-slate-400 transition-colors hover:text-amber-500" aria-label="Ubah">
                ...
                <button
                    ...
                    aria-label="Hapus"
                    data-student-dialog-open="delete-note-{{ $noteId }}"
                >
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
...
@if ($isEditableStudentNote && filled($noteId))
    <div class="teacher-dialog" data-student-dialog="delete-note-{{ $noteId }}" hidden>
        <div class="teacher-dialog__backdrop" data-student-dialog-close></div>
        <div class="teacher-dialog__panel">
            ...
            <p class="mb-6 text-center leading-relaxed text-slate-600">Catatan dan foto dalam entri ini akan dihapus secara permanen.</p>
            <form method="POST" action="{{ route('student.notes.destroy', $noteId) }}" class="teacher-dialog__form">
                @csrf
                @method('DELETE')
                <div class="mt-4 flex gap-3">
                    <button class="h-12 flex-1 rounded-full bg-slate-100 px-4 font-semibold text-slate-700 transition-colors hover:bg-slate-200" type="button" data-student-dialog-close>Batal</button>
...
<script>
    if (! window.__dmeStudentNoteCardInit) {
        window.__dmeStudentNoteCardInit = true;

        document.addEventListener('click', (event) => {
            const openLightbox = event.target.closest('[data-image-lightbox-open]');
            if (openLightbox) {
                const dialog = document.querySelector(`[data-image-lightbox="${openLightbox.dataset.imageLightboxOpen}"]`);
                if (dialog) {
                    dialog.hidden = false;
            ...
            const closeLightbox = event.target.closest('[data-image-lightbox-close]');
            if (closeLightbox) {
                const dialog = closeLightbox.closest('[data-image-lightbox]');
                if (dialog) {
                    dialog.hidden = true;
            ...
            const openDialog = event.target.closest('[data-student-dialog-open]');
            if (openDialog) {
                const dialog = document.querySelector(`[data-student-dialog="${openDialog.dataset.studentDialogOpen}"]`);
                if (dialog) dialog.hidden = false;
                return;
            }

            const closeDialog = event.target.closest('[data-student-dialog-close]');
            if (closeDialog) {
                const dialog = closeDialog.closest('[data-student-dialog]');
        ...
        document.addEventListener('keydown', (event) => {
            if (event.key === 'Escape') {
                ...
                document.querySelectorAll('[data-student-dialog]').forEach((dialog) => {
                    dialog.hidden = true;
                });
                document.body.style.overflow = '';
                document.body.classList.remove('dme-lightbox-open');
