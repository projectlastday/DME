...
@section('eyebrow', '')
@section('page_title', 'Detail Murid')

@section('content')
    <div class="dme-page-stack">
        <div class="transaction-page-heading">
            <h1 class="transaction-page-heading__title">Detail Murid</h1>
        </div>
        ...
        <section class="dme-section-card dme-section-stack">
            <div class="transaction-detail-grid">
                <div>
                    ...
                    <p class="transaction-row-card__value">{{ $student->name }}</p>
                </div>
                <div>
                    <p class="transaction-row-card__label">Role</p>
                    <p class="transaction-row-card__value">Murid</p>
                </div>
                <div>
                    <p class="transaction-row-card__label">ID User</p>
                    <p class="transaction-row-card__value">{{ $student->getKey() }}</p>
                </div>
            </div>

            <div class="transaction-detail-actions">
                <div class="transaction-detail-actions__inner">
                    <a href="{{ route('admin.students.edit', $student) }}" class="dme-button--secondary">Ubah</a>
                    <button type="button" class="dme-button--danger" data-teacher-dialog-open="delete-student-detail">Hapus</button>
                </div>
            </div>
        </section>
    </div>

    <div class="teacher-dialog" data-teacher-dialog="delete-student-detail" hidden>
        ...
        <div class="teacher-dialog__panel">
            ...
            <p class="mb-6 text-center leading-relaxed text-slate-600">Akun murid akan dihapus secara permanen.</p>
            <form method="POST" action="{{ route('admin.students.destroy', $student) }}" class="teacher-dialog__form">
                @csrf
                @method('DELETE')
